 ...
 */
export function stripBirthFoldSyntheticPrefix(content) {
    if (!content.startsWith(BIRTH_FOLD_TAG))
        return content;
...
function extractBirthFoldTokens(text) {
    const MAX = 8;
    const MAX_LEN = 60;
    const MIN_LEN = 4;
    const seen = new Set();
...
function safeJsonPreview(value, max) {
 ...
 */
export function convertLocalMessagesToSeedHistory(rows, options) {
    ...
    if (rows.length > 1) {
        ...
        for (let i = rows.length - 1; i >= 0; i--) {
            acc += typeof rows[i].tx === 'string' ? rows[i].tx.length + 32 : 96;
    ...
    const append = (role, part) => {
        ...
        if (last && last.role === role)
            ...
            blocks.push({ role, parts: [part] });
        usedRows += 1;
    };
    const renderBudgets = { maxToolInputChars, maxToolResultChars, maxMessageChars };
    ...
    let cutStart = 0;
    {
        ...
        if (cutStart > 0) {
            ...
            while (aligned < rendered.length && rendered[aligned].role !== 'user')
                aligned += 1;
            if (aligned < rendered.length)
                cutStart = aligned;
        }
    }
    ...
    for (let i = 0; i < cutStart; i++)
        droppedChars += rendered[i].content.length;
 ...
 */
function renderRowPart(row, budgets) {
    if (row.sg === true)
        return null;
    switch (row.ty) {
        case 'user': {
 ...
 */
export function convertLocalMessagesToTraceMessages(rows, options) {
    const maxChars = options?.maxChars ?? DEFAULT_BIRTH_FOLD_MAX_CHARS;
    const maxToolInputChars = options?.maxToolInputChars ?? DEFAULT_TOOL_INPUT_CHARS;
    const maxToolResultChars = options?.maxToolResultChars ?? DEFAULT_TOOL_RESULT_CHARS;
    const maxMessageChars = options?.maxMessageChars ?? DEFAULT_MESSAGE_CHARS;
    ...
    const budgets = { maxToolInputChars, maxToolResultChars, maxMessageChars };
    const rendered = [];
    for (const row of sourceRows) {
...
export function resolveBirthFoldMaxChars(raw) {
    if (!raw)
        return DEFAULT_BIRTH_FOLD_MAX_CHARS;
 ...
 */
export function resolveBirthFoldSeedMaxChars(effectiveWindowTokens, rawMaxChars, options = {}) {
    ...
    const optionCeiling = Number.isFinite(options.ceilingChars) && (options.ceilingChars ?? 0) > 0
        ? Math.floor(options.ceilingChars)
        : configuredCeiling;
    const ceiling = Math.min(configuredCeiling, optionCeiling);
    if (!Number.isFinite(effectiveWindowTokens) || (effectiveWindowTokens ?? 0) <= 0) {
        return ceiling;
    }
    const windowFitChars = Math.round(effectiveWindowTokens * BIRTH_FOLD_SEED_WINDOW_FRACTION * BIRTH_FOLD_SEED_CHARS_PER_TOKEN);
...
export function seedToAnthropicMessage(m) {
    return m.role === 'user'
...
export function seedToOpenAIChatMessage(m) {
    return { role: m.role, content: m.content };
}
/** Gemini history: model role + parts array. */
export function seedToGeminiContent(m) {
    return { role: m.role === 'assistant' ? 'model' : 'user', parts: [{ text: m.content }] };
}
//# sourceMappingURL=foldBirthHydration.js.map