...
export const DEFAULT_CONTEXT_BUDGET_CODEX_CLI_RECONSTRUCT_RUNWAY_TOKENS = DEFAULT_CONTEXT_BUDGET_TAIL_EPOCH_MIN_RUNWAY_TOKENS;
/**
 ...
 */
export const DEFAULT_CONTEXT_BUDGET_OPUS_MAX_PRESSURE_CEILING_TOKENS = DEFAULT_CONTEXT_BUDGET_PRESSURE_CEILING_TOKENS;
/**
 ...
 * provider wall.
 */
export const ENGINE_PRESSURE_CEILING_DEFAULTS = {
 ...
 */
export const MODEL_PRESSURE_CEILING_DEFAULTS = {};
...
function envAlias(env, voxxoKey, warpKey) {
    return env[voxxoKey] ?? env[warpKey];
}
function resolveFraction(option, envRaw, fallback) {
...
function clampPositiveTokensToWindow(tokens, windowTokens, fraction) {
...
function clampToCeiling(tokens, ceilingTokens, unsafeDevOverrides) {
    if (unsafeDevOverrides)
        return tokens;
    return ceilingTokens > 0 ? Math.min(tokens, ceilingTokens) : tokens;
}
function reserveFloor(windowTokens, fraction, min, max) {
    return Math.max(min, Math.min(max, Math.round(windowTokens * fraction)));
}
function classifyLimitSource(model, engine, explicitWindow) {
    if (explicitWindow)
    ...
    if (hasKnownModelShape)
        return 'model-or-engine-table';
    if (hasKnownEngine)
        return model ? 'model-or-engine-table' : 'engine-default';
    return 'conservative-fallback';
}
function classifyTier(windowTokens, source) {
    if (source === 'conservative-fallback')
        return 'unknown-conservative';
    if (windowTokens <= 128_000)
        return 'tiny-window';
    if (windowTokens <= 258_000)
        return 'small-200k';
    if (windowTokens <= 512_000)
        return 'mid-400k';
    if (windowTokens <= 1_048_576)
        return 'large-1m';
    return 'huge-2m';
}
function compressionProfileForTier(tier) {
    ...
    if (tier === 'mid-400k')
        return 'balanced';
    if (tier === 'large-1m')
        return 'cache-economic';
    return 'wide-cache-economic';
}
function defaultOutputReserveTokens(windowTokens) {
    if (windowTokens <= 128_000)
        return 8_000;
    if (windowTokens <= 258_000)
        return 16_000;
...
function defaultSystemToolsReserveTokens(windowTokens) {
    return reserveFloor(windowTokens, 0.08, 8_000, DEFAULT_CONTEXT_BUDGET_SYSTEM_TOOLS_RESERVE_TOKENS);
}
function defaultEmergencyMarginTokens(windowTokens) {
...
function defaultTailEpochPressureMarginTokens(windowTokens) {
...
function isGeminiCliEngine(engine) {
 ...
 */
function defaultPressureCeilingTokensForModelEngine(model, engine) {
...
function defaultCodexCliReconstructTriggerTokens(messageCeilingTokens, hardWindowTokens, pressureMaxWindowFraction) {
    const runwayCappedTarget = Math.max(1, messageCeilingTokens - DEFAULT_CONTEXT_BUDGET_CODEX_CLI_RECONSTRUCT_RUNWAY_TOKENS);
    return clampPositiveTokensToWindow(Math.min(DEFAULT_CONTEXT_BUDGET_FOLD_TRIGGER_TOKENS, runwayCappedTarget), hardWindowTokens, pressureMaxWindowFraction);
}
export function resolveContextBudget(input = {}) {
    ...
    const messageCeilingTokens = Math.max(1, hardWindowTokens - outputReserveTokens - emergencyMarginTokens);
    ...
    const bandMaxWindowFraction = resolveFraction(input.bandMaxWindowFraction, envAlias(env, 'VOXXO_FOLD_BAND_MAX_WINDOW_FRACTION', 'WARP_FOLD_BAND_MAX_WINDOW_FRACTION'), DEFAULT_CONTEXT_BUDGET_BAND_MAX_WINDOW_FRACTION);
    const bandWindowClamp = unsafeDevOverrides
        ? requestedBandTokens
        : clampPositiveTokensToWindow(requestedBandTokens, hardWindowTokens, bandMaxWindowFraction);
    const bandTokens = clampToCeiling(bandWindowClamp, messageCeilingTokens, unsafeDevOverrides);
    const pressureMaxWindowFraction = resolveFraction(input.pressureMaxWindowFraction, envAlias(env, 'VOXXO_FOLD_PRESSURE_MAX_WINDOW_FRACTION', 'WARP_FOLD_PRESSURE_MAX_WINDOW_FRACTION'), DEFAULT_CONTEXT_BUDGET_PRESSURE_MAX_WINDOW_FRACTION);
    const codexCliDefaultReconstructTriggerTokens = codexCliHardEpochOnly
        ? defaultCodexCliReconstructTriggerTokens(messageCeilingTokens, hardWindowTokens, pressureMaxWindowFraction)
        : null;
    let pressureCeilingTokens;
    ...
    if (input.pressureCeilingTokens === null || isDisabled(pressureCeilingEnv)) {
        ...
        const requestedPressure = positiveInt(input.pressureCeilingTokens)
            ...
            ?? clampPositiveTokensToWindow(defaultPressureCeilingTokensForModelEngine(model, engine), hardWindowTokens, pressureMaxWindowFraction);
    ...
    const singleCeilingFoldTriggerTokens = Math.max(1, pressureCeilingTokens ?? messageCeilingTokens);
    const foldTriggerUpperBound = Math.max(1, Math.min(pressureCeilingTokens ?? messageCeilingTokens, messageCeilingTokens) - tailEpochMinRunwayTokens);
    ...
    const appendOnlyMaxWindowFraction = resolveFraction(input.appendOnlyMaxWindowFraction, envAlias(env, 'VOXXO_FOLD_APPEND_ONLY_MAX_WINDOW_FRACTION', 'WARP_FOLD_APPEND_ONLY_MAX_WINDOW_FRACTION')
        ?? envAlias(env, 'VOXXO_FOLD_PREFIX_SATURATION_FRACTION', 'WARP_FOLD_PREFIX_SATURATION_FRACTION'), DEFAULT_CONTEXT_BUDGET_APPEND_ONLY_MAX_WINDOW_FRACTION);
    ...
    const tailEpochBandFraction = resolveFraction(input.tailEpochBandFraction, envAlias(env, 'VOXXO_FOLD_TAIL_EPOCH_BAND_FRACTION', 'WARP_FOLD_TAIL_EPOCH_BAND_FRACTION'), DEFAULT_CONTEXT_BUDGET_TAIL_EPOCH_BAND_FRACTION);
    ...
    const tailEpochCapTokens = clampToCeiling(Math.max(1, requestedTailEpochCapTokens), tailEpochCeiling, unsafeDevOverrides);
    const toolResultHeadroomSafety = resolveFraction(input.toolResultHeadroomSafety, envAlias(env, 'VOXXO_FOLD_TOOLRESULT_HEADROOM_SAFETY', 'WARP_FOLD_TOOLRESULT_HEADROOM_SAFETY'), DEFAULT_CONTEXT_BUDGET_TOOLRESULT_HEADROOM_SAFETY);
    const toolResultMinWindowFraction = resolveFraction(input.toolResultMinWindowFraction, envAlias(env, 'VOXXO_FOLD_TOOLRESULT_MIN_WINDOW_FRACTION', 'WARP_FOLD_TOOLRESULT_MIN_WINDOW_FRACTION'), DEFAULT_CONTEXT_BUDGET_TOOLRESULT_MIN_WINDOW_FRACTION);
    ...
    const floorChars = Math.min(Math.round(hardWindowTokens * charsPerToken * toolResultMinWindowFraction), batchAvailableChars);
    ...
    const evictionPolicy = compressionProfile === 'survival'
        ? 'hard-epoch-only'
