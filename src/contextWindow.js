...
function isCodexApiLargeContextModel(modelLower) {
    if (!modelLower)
        return false;
...
function contextWindowOverrideForEngineModel(modelLower, engineLower) {
 ...
 */
export function contextWindowForModel(model, engine) {
    ...
    if (engineModelOverride !== undefined)
        return engineModelOverride;
    // 2. Exact match
 ...
 */
export function getUtilizationLevel(currentTokens, contextWindow) {
    if (contextWindow <= 0)
        return 'healthy';
    const pct = currentTokens / contextWindow;
    if (pct >= CONTEXT_THRESHOLDS.AUTO_COMPACT)
        return 'auto_compact';
    if (pct >= CONTEXT_THRESHOLDS.CRITICAL)
        return 'critical';
    if (pct >= CONTEXT_THRESHOLDS.WARNING)
        return 'warning';
    return 'healthy';
}
/**
 ...
 */
export function estimateTurnsRemaining(currentTokens, contextWindow, avgTokensPerTurn) {
    if (contextWindow <= 0 || avgTokensPerTurn <= 0)
        return Infinity;
