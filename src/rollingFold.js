...
// Defaults
// ══════════════════════════════════════════════════════════════════════
export const DEFAULT_ASSISTANT_TEXT_BUDGET = {
    fullRetentionChars: 50_000,
    essenceRetentionChars: 100_000,
};
export const DEFAULT_FOLD_CONFIG = {
 ...
 */
export const ALWAYS_ON_FOLD_CONFIG = {
    ...DEFAULT_FOLD_CONFIG,
    continuous: true,
...
export const DEFAULT_FIDELITY_VALUE_WEIGHTS = {
    read: 1,
    claim: 3,
    edit: 4,
    userNamed: 5,
 ...
 */
export function resolveFoldBandBudgets(bandTokens, charsPerToken = BAND_CHARS_PER_TOKEN, fidelity) {
 ...
 */
export function resolveFoldConfigForBand(bandTokens = DEFAULT_FOLD_BAND_TOKENS, charsPerToken = BAND_CHARS_PER_TOKEN, fidelity) {
 ...
 */
export function resolveColdFoldConfigForBand(bandTokens = DEFAULT_FOLD_BAND_TOKENS, charsPerToken = BAND_CHARS_PER_TOKEN, fidelity) {
    const resolvedBandTokens = bandTokens ?? DEFAULT_FOLD_BAND_TOKENS;
    const band = resolveFoldBandBudgets(resolvedBandTokens, charsPerToken, fidelity);
    ...
    return {
        ...
        verbatimKeepChars: Math.min(baseVerbatimKeepChars, Math.max(1000, Math.round(band.bandChars * 0.05))),
    };
}
// ══════════════════════════════════════════════════════════════════════
// Internal helpers
// ══════════════════════════════════════════════════════════════════════
export function countChars(messages) {
    let total = 0;
 ...
 */
export function countCharsByKind(messages) {
    let toolChars = 0;
    let textChars = 0;
    for (const msg of messages) {
 ...
 */
export function stripRecallCardBlocks(text) {
    let result = text;
...
const EMPTY_SYNTHETIC_CONTEXT_OPTIONS = Object.freeze({});
/**
...
export function formatFoldEpochStamp(epoch, detail) {
 ...
 */
export function extractCoordinateConservationCorpus(messages) {
    const conserved = [];
 ...
 */
export function dedupeCoordinateClosetText(text, priorCoordinateCorpus) {
    if (!text.includes(COORDINATE_CLOSET_MARKER) || !priorCoordinateCorpus)
        return text;
    return text
        .split('\n')
        .map((line) => {
...
function matchesHostSyntheticContext(text, options) {
    for (const prefix of options.prefixes ?? []) {
        if (text.startsWith(prefix))
            return true;
    }
    for (const block of options.leadingBlocks ?? []) {
        if (text.startsWith(block.prefix))
            return true;
    }
 ...
 */
export function isSyntheticContextText(text, syntheticContext = EMPTY_SYNTHETIC_CONTEXT_OPTIONS) {
...
export function stripUserMessageVaultBlocks(text) {
    let result = text;
...
function stripOneLeadingSyntheticUserContextBlock(text, syntheticContext) {
    ...
    for (const block of syntheticContext.leadingBlocks ?? []) {
        if (!body.startsWith(block.prefix))
            continue;
        switch (block.mode) {
            case 'line':
...
export function stripSyntheticUserContextBlocks(text, syntheticContext = EMPTY_SYNTHETIC_CONTEXT_OPTIONS) {
...
function isUserTurnBoundary(msg, syntheticContext) {
    if (msg.role !== 'user')
        return false;
...
export function detectTurns(messages, syntheticContext = EMPTY_SYNTHETIC_CONTEXT_OPTIONS) {
    const turns = [];
    let turnStart = -1;
...
function isStepBoundary(msg) {
    if (msg.role !== 'assistant' && msg.role !== 'model')
        return false;
    // Anthropic format: assistant content blocks with type tool_use.
    if (Array.isArray(msg.content) && msg.content.some(b => b?.type === 'tool_use'))
        return true;
    ...
    const parts = msg.parts;
    if (Array.isArray(parts) && parts.some((p) => p?.functionCall))
        return true;
    return false;
}
/**
 ...
 */
function segmentTurnBySteps(turn) {
 ...
 */
export function planActiveTurnStepFold(messages, opts, syntheticContext = EMPTY_SYNTHETIC_CONTEXT_OPTIONS) {
    ...
    if (activeChars < opts.activeTurnCharBudget)
        return null;
    ...
    if (segments.length <= minSegmentsToFold)
        return null;
    ...
    if (turnsToFold <= priorTurns.length)
        return null;
    return { turns: allTurns, turnsToFold };
}
function extractToolCalls(turnMessages) {
    const calls = [];
                    ...
                    if (p) {
                        const rt = typeof block.content === 'string'
                            ...
                            : Array.isArray(block.content)
                                ? block.content.map((b) => typeof b === 'string' ? b : b?.text ?? JSON.stringify(b)).join('\n')
...
export function extractAssistantText(turnMessages) {
    const texts = [];
    for (const msg of turnMessages) {
        if (msg.role !== 'assistant' && msg.role !== 'model')
            continue;
 ...
 */
export function extractUserText(turnMessages, syntheticContext = EMPTY_SYNTHETIC_CONTEXT_OPTIONS) {
    const texts = [];
    for (const msg of turnMessages) {
        if (msg.role !== 'user')
            continue;
 ...
 * back to new Date()).
 */
function extractTurnTsMs(messages) {
    for (const msg of messages) {
 ...
 */
export function extractUserNamedPaths(text) {
    const seen = new Set();
    const result = [];
 ...
 */
export function nominateVerbatim(text, cap = 40) {
    const seen = new Set();
    const result = [];
    const addMatch = (raw) => {
 ...
 */
export function isConservedIn(haystack, value) {
    if (boundaryHit(haystack, value))
        return true;
    const valuePart = /[=:][ ]?(.+)$/.exec(value)?.[1]?.trim() ?? value;
    if (valuePart !== value && boundaryHit(haystack, valuePart))
        return true;
    const norm = normalizeNumericForm(valuePart);
    if (norm !== valuePart && boundaryHit(haystack, norm))
        return true;
    return false;
}
/** Canonical comparison key for Coordinate Closet literals. */
export function canonicalizeClosetLiteral(value) {
    const trimmed = value.trim();
    return trimmed.startsWith('/') ? trimmed.slice(1) : trimmed;
}
function preferredClosetLiteral(left, right) {
    if (left.startsWith('/') !== right.startsWith('/'))
        return left.startsWith('/') ? left : right;
    return left.length >= right.length ? left : right;
}
function pathSuffixContains(container, contained) {
 ...
 */
export function admitClosetLiteral(admitted, candidate) {
    ...
    for (let i = 0; i < admitted.length; i += 1) {
        ...
        if (pathSuffixContains(existingKey, key))
            return false;
    }
 ...
 */
export function extractVerbatimContextLabel(sourceText, value) {
    ...
    if (!/[A-Za-z]/.test(label) || /^[0-9a-fA-F]+$/.test(label))
        return '';
    return label.slice(0, LABEL_MAX_CHARS);
}
/** Reject opaque values only when no source-derived label or explicit key exists. */
export function isUnlabeledOpaqueClosetLiteral(value) {
    const v = value.trim();
    if (!v)
        return true;
    if (/\s\([^)]+\)$/.test(v))
        return false;
    if (/^[A-Za-z_][\w.-]{0,40}[=:]/.test(v))
        return false;
...
function slashWordSegments(value) {
    if (!value.includes('/'))
        return [];
 ...
 */
export function isClosetNoiseLiteral(value) {
    const v = value.trim();
    if (!v)
        return true;
    // ── Tool / trace artifact paths (any engine, any lineage) — pure exhaust ──
    if (v.includes('tool-result-spool/'))
        return true;
    // Browser artifact captures (screenshots / DOM dumps).
    if (v.includes('browser-artifacts/'))
        return true;
    if (/(?:^|\/)scr_\d{6,}_[0-9a-f]{6,}/.test(v))
        return true;
    // Scratch / temp paths.
    if (/(?:^|\/)tmp\//.test(v))
        return true;
    ...
    if (/^\d{1,4}\/\d{1,4}$/.test(v))
        return true;
    if (/^[A-Za-z_][\w.-]{0,40}[=:][ ]?\d{1,4}\/\d{1,4}$/.test(v))
        return true;
    if (/^\d{1,2}\/\d{1,2}\/\d{2,4}$/.test(v))
        return true;
    if (/^\d{1,4}\/[A-Za-z]{2,}$/.test(v))
        return true;
    ...
    if (/^[A-Za-z]{2,}\/[A-Za-z]{2,}$/.test(v) && /[A-Z]/.test(v))
        return true;
    if (isLowSignalSlashChain(v))
        return true;
    if (hasSingleCharNoiseSegment(v))
        return true;
 ...
 */
function beltVerbatim(text, max = 2) {
    const picked = [];
    for (const lit of nominateVerbatim(text, max * 4)) {
        ...
        if (isClosetNoiseLiteral(lit))
            continue;
        if (picked.length > 0 && isConservedIn(picked.join(', '), lit))
            continue;
        picked.push(lit);
    }
    return picked.join(', ');
...
function isMutatingBash(input) {
    return MUTATING_BASH_RE.test(String(input.command ?? ''));
}
export function classifyTurn(turnMessages) {
    ...
    const hasCommandError = toolCalls.some(tc => tc.name === 'Bash' && (tc.resultText.includes('Error:') || tc.resultText.includes('FAILED')));
    if (hasCommandError && toolCalls.every(tc => tc.name === 'Bash'))
        return 'error';
    ...
    if (actionCount > 0 || bashMutations > 0)
        return 'action';
    ...
    if (coordCount > 0 && researchCount === 0 && bashReads === 0)
        return 'coordination';
    if (researchCount > 0 || bashReads > 0)
        return 'research';
    if (assistantText.length < 100 && toolNames.length === 0)
        return 'navigation';
    return 'decision';
}
// ══════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════
export function skeletonizeTool(call) {
    ...
    switch (name) {
        ...
        default: {
            if (shortName === 'atlas_query') {
                ...
                if (action === 'lookup')
                    return `🗺️ atlas lookup ${path}`;
                if (action === 'search')
                    return `🗺️ atlas search "${query}"`;
                if (action === 'plan_context')
                    return `🗺️ atlas plan "${query}"`;
                return `🗺️ atlas ${action} ${path || query}`.trim();
            }
            if (shortName === 'atlas_commit')
                return `📌 atlas_commit ${extractPath(input)}`;
            if (shortName === 'atlas_graph')
                return `🗺️ atlas_graph ${String(input.kind ?? '')} ${extractPath(input)}`.trim();
            ...
            if (shortName === 'tap_instance_messages')
                return `👂 tap ${String(input.target_instance ?? '')}`;
            if (shortName === 'task_rail')
                return `📋 task_rail ${String(input.mode ?? '')} ${String(input.operation ?? '')}`.trim();
            if (shortName === 'self_rebirth' || shortName === 'brain_rebirth')
                return `🔄 rebirth`;
...
export function extractAssistantEssence(text) {
    ...
    for (const line of lines) {
        ...
        if (KEEP_RE.some(p => p.test(trimmed))) {
            kept.push(line);
            continue;
        }
        // Drop filler lines
        if (FILLER_RE.some(p => p.test(trimmed)))
            continue;
...
export function collapseSequences(foldedTurns) {
    if (foldedTurns.length <= 1)
        return foldedTurns;
    const result = [];
    let i = 0;
    while (i < foldedTurns.length) {
        ...
        let seqEnd = i + 1;
        while (seqEnd < foldedTurns.length &&
...
// Trigger mechanism
// ══════════════════════════════════════════════════════════════════════
export function checkFoldTrigger(messages, config = DEFAULT_FOLD_CONFIG, syntheticContext = EMPTY_SYNTHETIC_CONTEXT_OPTIONS) {
...
export function formatFoldTombstoneLine(span) {
...
export function mergeEvictionSpans(spans, maxSpans = MAX_TOMBSTONE_SPANS) {
 ...
 */
export function computeEvictableThroughOrdinal(turns, durableCursorIndex, epochFoldFrontiers, upcomingEpoch) {
    let cursorOrdinal = 0;
    for (const turn of turns) {
        if (turn.endIndex <= durableCursorIndex)
            cursorOrdinal++;
        else
            break;
    }
...
// Fold orchestrator
// ══════════════════════════════════════════════════════════════════════
function renderFoldedBlock(collapsed, stats, closetLine, tombstoneLines, counterStamp, preamble = FOLD_BLOCK_PREAMBLE) {
    const lines = [
...
// ══════════════════════════════════════════════════════════════════════
const DURABLE_GLYPH_PREFIXES = ['🏁', '⚠️', '⚠'];
/**
 ...
 */
function turnOpensWithDurableGlyph(assistantText) {
    return DURABLE_GLYPH_PREFIXES.some(g => assistantText.startsWith(g));
}
const TRANSIENT_GLYPH_PREFIXES = ['🔍', '▶'];
/**
 ...
 */
function turnOpensWithTransientGlyph(assistantText) {
    return TRANSIENT_GLYPH_PREFIXES.some(g => assistantText.startsWith(g));
}
export function resolveFidelityValueWeights(overrides) {
...
function fidelityPathKey(rawPath) {
    return rawPath.replace(/:\d+(?:-\d+)?$/, '');
}
function fidelityRefKind(toolName) {
    if (toolName === 'Edit' || toolName === 'Write' || toolName === 'NotebookEdit')
        return 'edit';
    if (toolName.includes('claim_file'))
        return 'claim';
    return 'read';
}
/**
 ...
 */
export function scoreTurnFidelityValue(turns, foldCount, weights = DEFAULT_FIDELITY_VALUE_WEIGHTS, syntheticContext = EMPTY_SYNTHETIC_CONTEXT_OPTIONS) {
    const n = turns.length;
    const fold = Math.max(0, Math.min(Math.floor(foldCount), n));
    const turnPaths = new Array(n);
    for (let g = 0; g < n; g++) {
        const refs = [];
    ...
    for (let g = 0; g < n; g++) {
        for (const r of turnPaths[g]) {
            ...
            if (arr)
                arr.push({ g, kind: r.kind });
            else
                refsByKey.set(r.key, [{ g, kind: r.kind }]);
        }
    }
    const kindWeight = (kind) => kind === 'userNamed' ? weights.userNamed
    ...
    for (let i = 0; i < fold; i++) {
        ...
        for (const { key } of turnPaths[i]) {
            if (seen.has(key))
                continue; // dedupe a turn's own repeated path
            seen.add(key);
            ...
            for (const ref of refs) {
                if (ref.g <= i)
                    continue; // downstream references only
                score += kindWeight(ref.kind) * (ref.g >= fold ? weights.activeWindowMultiplier : 1);
                if (opensDurable[ref.g])
                    supersededByDurable = true;
            }
        }
        if (opensDurable[i]) {
 ...
 */
export function foldContext(messages, turnsToFold, config = DEFAULT_FOLD_CONFIG, eviction, counterStamp, precomputedTurns, syntheticContext = EMPTY_SYNTHETIC_CONTEXT_OPTIONS, fidelityValue) {
    ...
    const initialEvictedThrough = eviction
        ? Math.min(eviction.evictedSpans.reduce((max, s) => Math.max(max, s.toOrdinalExclusive), 0), actualFoldCount)
        : 0;
    ...
    const retentionLevels = new Array(turnsToCompress.length).fill('skeleton');
    ...
    if (!fidelityValue) {
        ...
        for (let j = turnsToCompress.length - 1; j >= 0; j--) {
            if (j < initialEvictedThrough)
                continue; // evicted ordinals never consume budget
        ...
        const recencyFloor = Math.max(0, Math.floor(fidelityValue.recencyFloorTurns ?? DEFAULT_FIDELITY_VALUE_RECENCY_FLOOR_TURNS));
        ...
        const order = [];
        for (let j = turnsToCompress.length - 1; j >= floorStart; j--)
            order.push(j); // recency floor, newest-first
        const rest = [];
        for (let j = initialEvictedThrough; j < floorStart; j++)
            rest.push(j);
    ...
    for (let turnIdx = 0; turnIdx < turnsToCompress.length; turnIdx++) {
        if (turnIdx < initialEvictedThrough)
            continue; // tombstoned at a prior epoch
        ...
        let retained;
        const shouldRetainNewestUserText = config.retainNewestUserTextInFoldBlock !== false &&
    ...
    const assembleBlock = (evictedThroughOrdinal, spans) => {
        ...
        if (closetBudget > 0) {
            ...
            const admit = (lit, sourceText, ceiling) => {
                if (closetSet.has(lit))
                    return;
                ...
                if (isClosetNoiseLiteral(lit))
                    return;
                if (priorCoordinateCorpus && isConservedIn(priorCoordinateCorpus, lit))
                    return;
                if (isConservedIn(bodyCorpus, lit))
                    return;
                if (closetItems && isConservedIn(closetItems, lit))
                    return;
            ...
            const userLaneCeiling = Math.min(closetBudget, closetItems.length + Math.floor(closetBudget * USER_VERBATIM_LANE_RATIO));
            for (const built of survivors) {
                if (!built.userNominationText)
                    continue;
        ...
        const blockChars = collapsed.reduce((s, ft) => s + ft.skeleton.length + (ft.retained?.length ?? 0), 0) +
            (closetLine?.length ?? 0) +
    ...
    // sawtooth behavior.
    let evictionSpans = eviction ? eviction.evictedSpans.map(s => ({ ...s })) : [];
    let evictedThrough = initialEvictedThrough;
    let evictionOutcome;
    const spansFor = (through) => {
        if (through <= initialEvictedThrough)
            return mergeEvictionSpans(evictionSpans);
    ...
    if (eviction && eviction.thresholdChars > 0) {
        ...
        if (requestedTarget !== undefined) {
            ...
            if (evictedThrough > initialEvictedThrough)
    ...
    const foldedMessage = { role: 'user', content: foldedBlockText };
    const ackMessage = {
        role: 'assistant',
...
export const DEFAULT_INTRA_FOLD_CONFIG = {
    tailBuffer: 5,
    minTruncateSize: 500,
 ...
 */
export const ALWAYS_ON_INTRA_FOLD_CONFIG = {
    tailBuffer: 5,
    minTruncateSize: 2_000,
    charThreshold: 0,
    atlasLookupThreshold: 8_000,
};
function geminiFunctionResponseText(response) {
...
function withFoldedGeminiFunctionResponse(response, foldedText) {
    if (response && typeof response === 'object' && !Array.isArray(response)) {
        return { ...response, result: foldedText };
    }
    return { result: foldedText };
}
function buildToolInfoMap(turnMessages) {
    const map = new Map();
    for (const msg of turnMessages) {
        if (msg.role !== 'assistant' && msg.role !== 'model')
            continue;
...
function isAtlasQueryTool(name) {
...
function isClaimedPath(toolPath, claimedPaths) {
    if (!claimedPaths || claimedPaths.size === 0)
        return false;
    ...
    for (const claimed of claimedPaths) {
        ...
        if (normalized === normalizedClaimed || toolPath === claimed)
            return true;
    }
    return false;
}
 ...
 */
export function extractToolPathSet(messages) {
    const paths = new Set();
    for (const msg of messages) {
        if (msg.role !== 'assistant' && msg.role !== 'model')
            continue;
 ...
 */
function foldAtlasPreservingMetadata(content, info, charCount) {
    if (!info || !isAtlasQueryTool(info.name))
        return null;
...
function findToolResults(turnMessages) {
    const refs = [];
...
function foldSummaryText(info, charCount, originalContent) {
...
function foldTurnToolResults(turnMessages, config) {
    ...
    for (let i = 0; i < turnMessages.length; i++) {
        ...
        if (msg.role === 'user' && Array.isArray(msg.parts)) {
            ...
            foldedMessages.push(changed ? { ...msg, parts: newParts } : msg);
            continue;
        }
 ...
 */
export function intraTurnFold(messages, config = DEFAULT_INTRA_FOLD_CONFIG, syntheticContext = EMPTY_SYNTHETIC_CONTEXT_OPTIONS) {
    const originalChars = countChars(messages);
    ...
    let totalFolded = 0;
    let totalKept = 0;
    const result = [...messages.slice(0, prefixEnd)];
    for (const turn of turns) {
...
//# sourceMappingURL=rollingFold.js.map