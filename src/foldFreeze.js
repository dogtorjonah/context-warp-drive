 ...
 */
import { countChars, extractToolPathSet, normalizeToolPath, } from './rollingFold.ts';
import { buildRawRebirthSeedFromMessages, DEFAULT_RAW_REBIRTH_SEED_PACKAGE_BUDGET_CHARS, DEFAULT_RAW_REBIRTH_SEED_SECTION_MAX_CHARS, findRawRebirthSeedTraceEnd, } from './rawRebirthSeed.ts';
export const DEFAULT_FOLD_FREEZE_CONFIG = {
    enabled: true,
    ttlMs: 5 * 60_000,
 ...
 */
function boundaryFingerprintInput(msg) {
 ...
 */
export function isTailEpochEfficiencyAlarm(shrinkRatio) {
 ...
 */
export function shouldEscalateTailEpochForLowYield(shrinkRatio, measuredInputTokens, foldTriggerTokens, tailEpochMinRunwayTokens) {
...
export const FOLD_FREEZE_HARD_EPOCH_CAUSES = [
    'first-call',
    'cold-gap',
    'context-changed',
    'history-rewound',
 ...
 */
export function buildRawHardEpochSeed(messages, options = {}) {
    ...
    const closetBudget = isCompact
        ? Math.min(options.closetChars ?? DEFAULT_RAW_HARD_EPOCH_CLOSET_CHARS, Math.max(700, Math.floor(maxChars * 0.18)))
 ...
 */
export function buildHardEpochSeedView(messages, seedPrompt) {
...
function ensureHardEpochContinuityDirective(seedPrompt) {
 ...
 */
function extractTrailingUserTurnText(messages, traceEnd) {
    const parts = [];
...
function findTrailingUserRunStart(messages) {
    let i = messages.length;
    while (i > 0 && messages[i - 1]?.role === 'user')
        i -= 1;
    return i;
}
function findRawHardEpochTraceEnd(messages) {
    return findRawRebirthSeedTraceEnd(messages, false);
}
export function createFoldFreezeState() {
    return {
        frozenView: null,
        frozenViewChars: 0,
...
export function serializeFoldFreezeState(state) {
    return {
        version: 1,
...
export function restoreFoldFreezeState(snapshot) {
    return {
...
export function getFoldFreezeMetadata(state) {
    return {
 ...
 */
export function summarizeFrozenBands(state) {
    const bands = state.sealedBands.map((band, index) => ({
        bandIndex: index,
    ...
    const seedBaseChars = state.sealedBands.length > 0 ? state.sealedBands[0].sealedPrefixChars : state.frozenViewChars;
 ...
 * recomputing.
 */
export function evaluateFoldFreeze(state, history, context, now, config) {
...
export function touchFoldFreeze(state, now) {
 ...
 */
export function initializeFoldFreezeBase(state, history, view, context, now) {
 ...
 */
export function commitFoldFreeze(state, history, view, context, now, hardEpochCause = 'first-call') {
 ...
 */
export function appendFoldFreezeTailEpoch(state, history, tailView, context, now) {
    if (!state.frozenView) {
        return {
            committed: false,
            view: null,
...
//# sourceMappingURL=foldFreeze.js.map