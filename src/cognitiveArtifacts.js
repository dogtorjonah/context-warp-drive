 ...
 */
import { parseRegisterGlyph, CARD_GLYPHS, } from './glyphs.ts';
...
const DURABLE_REGISTERS = new Set([
    'verdict',
    'hazard',
    'blocked',
]);
/** Registers admitted through the transient flow-note lane. */
const TRANSIENT_REGISTERS = new Set([
    'in_progress',
    'executing',
]);
 ...
 */
const SYNTHETIC_NOISE_PREFIXES = ['API Error', '[Request interrupted'];
// ══════════════════════════════════════════════════════════════════════
// Helpers
// ══════════════════════════════════════════════════════════════════════
/** Glyphs by register for rendering. */
const REGISTER_GLYPHS = {
    verdict: '🏁',
    hazard: '⚠️',
    blocked: '❓',
    executing: '▶',
 ...
 */
function flattenContent(content, parts) {
 ...
 */
function flattenPartsArray(parts) {
 ...
 */
export function flattenFoldMessageText(content, parts) {
 ...
 */
export function extractCognitiveArtifacts(messages, options = {}) {
    const includeFlowNotes = options.includeFlowNotes !== false;
    const durable = [];
    const flowNotes = [];
    for (let i = 0; i < messages.length; i++) {
        const msg = messages[i];
        if (!msg || !ASSISTANT_ROLES.has(msg.role))
            continue;
    ...
    const cappedDurable = durable.length > MAX_ARTIFACTS ? durable.slice(-MAX_ARTIFACTS) : durable;
    const cappedFlowNotes = flowNotes.length > MAX_FLOW_NOTES ? flowNotes.slice(-MAX_FLOW_NOTES) : flowNotes;
    return [...cappedDurable, ...cappedFlowNotes].sort((a, b) => a.messageIndex - b.messageIndex);
}
/**
 ...
 */
export function renderCognitiveBlock(artifacts) {
    ...
    const lines = artifacts.flatMap((a) => [formatCognitiveArtifactProvenance(a), `${a.glyph} ${a.headline}`]);
 ...
 * injection cache.
 */
export function formatCognitiveArtifactProvenance(artifact) {
 ...
 */
export function enrichFoldedBandBody(parts, rawMessages, options) {
 ...
 */
export function mergeBlockIntoViewTail(view, block) {
    if (!block)
        return view.slice();
    if (view.length === 0)
        return view.slice();
    ...
    return [...view.slice(0, -1), { ...last, content }];
}
//# sourceMappingURL=cognitiveArtifacts.js.map