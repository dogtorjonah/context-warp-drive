...
export function normalizeRecallUtility(value) {
    if (typeof value !== 'number' || !Number.isFinite(value) || value < 0)
        return undefined;
    return Math.min(1, Math.max(0, value));
}
export function recallUtilityMultiplier(value) {
...
export function recallUtilityDebugFields(value) {
    const utility = normalizeRecallUtility(value);
    if (utility === undefined)
        return undefined;
...
const TOUCH_KIND_RANK = { edit: 2, read: 1, mention: 0 };
 ...
 * the same rule.
 */
export function isEpisodeMemberPath(candidate) {
    ...
    if (!lastSegment.includes('.'))
        return false;
    if (candidate === '/bin' || candidate.startsWith('/bin/')
...
export function truncateVerbatim(text, cap) {
    if (text.length <= cap)
        return text;
    if (cap <= 1)
        return '…';
 ...
 */
export function extractRationaleLines(text, isSyntheticLine, cap = RATIONALE_MAX_LINES) {
    if (!text)
        return [];
    const out = [];
    ...
    for (const rawLine of text.split('\n')) {
        ...
        if (inCodeBlock || trimmed.length === 0)
            continue;
        if (isSyntheticLine(trimmed) || NARRATION_QUOTED_VOICE_RE.test(trimmed) || NARRATION_NONVERDICT_LINE_RE.test(trimmed))
            continue;
        const stripped = trimmed.replace(NARRATION_DECORATION_RE, '');
        if (stripped.length < NARRATION_MIN_LINE_CHARS || stripped.length > NARRATION_MAX_LINE_CHARS)
            continue;
        if (stripped.endsWith('?'))
            continue;
        if (!RATIONALE_RE.test(stripped))
            continue;
 ...
 */
export function extractProcessNarrationLines(text, messageMode, isSyntheticLine, toolStepCount, cap = PROCESS_MAX_LINES) {
    if (!text || toolStepCount < PROCESS_MIN_TOOL_STEPS || cap <= 0)
        return [];
    if (messageMode === 'blocked' || messageMode === 'verdict' || messageMode === 'hazard')
        return [];
    const out = [];
    ...
    for (const rawLine of text.split('\n')) {
        ...
        if (lineMode === 'blocked' || lineMode === 'verdict' || lineMode === 'hazard')
            continue;
        if (isSyntheticLine(trimmed) || NARRATION_QUOTED_VOICE_RE.test(trimmed))
            continue;
        ...
        if (stripped.endsWith('?'))
            continue;
        let kind = null;
        if (PROCESS_DECISION_RE.test(stripped))
            kind = 'process:decision';
        else if (PROCESS_DISCOVERY_RE.test(stripped))
            kind = 'process:discovery';
        else if (toolStepCount >= PROCESS_INVESTIGATION_MIN_TOOL_STEPS
            && lineMode === 'working'
 ...
 */
export function classifyMessageGlyph(text) {
    if (!text)
        return undefined;
 ...
 */
export function isNarrationEligibleGlyph(mode) {
 ...
 */
export function narrationKindForGlyph(mode) {
    if (mode === 'verdict')
        return 'narration:verdict';
    if (mode === 'hazard')
        return 'narration:hazard';
    return 'narration';
}
/**
 ...
 */
export function annotationsImplyGotcha(annotations) {
 ...
 */
export function extractNarrationLines(text, isSyntheticLine, cap = NARRATION_MAX_LINES, options) {
    if (!text)
        return [];
    ...
    for (const rawLine of text.split('\n')) {
        ...
        if (stripped.endsWith('?'))
            continue;
        if (requireVerdictShape && !NARRATION_VERDICT_RE.test(stripped))
            continue;
        out.push(truncateVerbatim(stripped, VOICE_TEXT_CAP_CHARS));
    }
    return out;
...
function episodeEventRange(episode) {
    let first = Number.POSITIVE_INFINITY;
    let last = Number.NEGATIVE_INFINITY;
    for (const member of episode.members) {
        if (member.firstSeen < first)
            first = member.firstSeen;
        if (member.lastSeen > last)
            last = member.lastSeen;
    }
 ...
 */
export function groupTouchesIntoEpisodes(touches, opts = {}) {
    ...
    const sorted = touches
        ...
        .sort((a, b) => a.eventIndex - b.eventIndex || comparePaths(a.path, b.path) || TOUCH_KIND_RANK[b.kind] - TOUCH_KIND_RANK[a.kind]);
    ...
    const burstHasVoice = (burstTouches, prevBurstEnd) => {
        if (!voiceIdxSet && !intentIdxSet)
            return true; // no voice data → never hold
    ...
    const burstHasEvent = (burstTouches, eventSet) => {
        for (const t of burstTouches) {
            if (eventSet.has(t.eventIndex))
                return true;
        }
        return false;
    };
    ...
    for (const touch of sorted) {
        ...
        if (prev) {
            while (pivotCursor < pivotIndexes.length && pivotIndexes[pivotCursor] <= prev.eventIndex)
                pivotCursor++;
            const pivotBetween = pivotCursor < pivotIndexes.length && pivotIndexes[pivotCursor] <= touch.eventIndex;
            while (railSealCursor < railSealIndexes.length && railSealIndexes[railSealCursor] <= prev.eventIndex)
                railSealCursor++;
...
function assembleBurst(burstTouches, memberCap) {
    const byPath = new Map();
    for (const touch of burstTouches) {
        const existing = byPath.get(touch.path);
        if (existing) {
            existing.touchCount += 1;
            if (TOUCH_KIND_RANK[touch.kind] > TOUCH_KIND_RANK[existing.touchKind])
                existing.touchKind = touch.kind;
            if (touch.eventIndex < existing.firstSeen)
                existing.firstSeen = touch.eventIndex;
            if (touch.eventIndex > existing.lastSeen)
                existing.lastSeen = touch.eventIndex;
        }
        else {
    ...
    if (members.length > memberCap) {
        members = members
            .sort((a, b) => TOUCH_KIND_RANK[b.touchKind] - TOUCH_KIND_RANK[a.touchKind]
 ...
 */
export function assignAnnotationsToBursts(bursts, annotated) {
    const result = bursts.map(() => []);
    if (bursts.length === 0)
        return result;
    ...
    for (const { eventIndex, annotation } of sorted) {
        ...
        for (let i = 0; i < bursts.length; i++) {
            if (eventIndex >= bursts[i].startEventIndex)
                target = i;
            else
                break;
        }
...
function renderStructuralStep(step) {
    ...
    if (step.result)
        token += ` ⇢ "${truncateVerbatim(step.result, TRACE_RESULT_TEXT_CAP_CHARS)}"`;
    return token;
}
function renderVoiceInline(annotation) {
    const text = truncateVerbatim(annotation.text, TRACE_VOICE_TEXT_CAP_CHARS);
    if (annotation.kind.startsWith('star:'))
        return `⭐${annotation.kind.slice(5)}:"${text}"`;
    if (annotation.kind === 'changelog')
        return `✎:"${text}"`;
    if (annotation.kind.startsWith('narration'))
        return `🗣:"${text}"`;
    if (annotation.kind === 'rail')
        return `🛤:"${text}"`;
    return `💬:"${text}"`;
}
function structuralKey(step) {
    return `${step.tool}\u0000${step.target ?? ''}`;
}
function fullKey(step) {
    return `${structuralKey(step)}\u0000${step.outcome ?? ''}\u0000${step.detail ?? ''}\u0000${step.result ?? ''}`;
}
function traceTokenPriority(step) {
    if (step.result)
        return 100;
    ...
    if (step.outcome === 'ok')
        return 45;
    if (/edit|write|patch|commit/i.test(step.tool))
        return 25;
    return 10;
}
function truncateEvidenceToken(token, capChars) {
    ...
    if (marker < 0)
        return truncateVerbatim(token, cap);
    const evidence = token.slice(marker);
    if (evidence.length >= cap)
        return truncateVerbatim(evidence, cap);
 ...
 */
export function buildBranchTrace(steps, capChars = BRANCH_TRACE_CAP_CHARS) {
    if (steps.length === 0)
        return '';
    ...
    while (i < steps.length) {
        ...
        let run = 1;
        while (i + run < steps.length && !steps[i + run].voice && fullKey(steps[i + run]) === fullKey(step))
            run += 1;
    ...
    const join = (parts) => parts.map((t) => t.text).join(' → ');
    if (join(tokens).length <= cap)
        return join(tokens);
    ...
    const renderSelection = (selected) => {
        ...
        const trailing = tokens
            ...
            .reduce((total, token) => total + token.stepCount, 0);
        if (trailing > 0)
            parts.push(`… ⟨${trailing} steps⟩ …`);
        return parts.join(' → ');
    };
    const selected = new Set([0, tokens.length - 1]);
    let anchor = -1;
    for (let index = 1; index < tokens.length - 1; index++) {
        if (anchor < 0 || tokens[index].priority > tokens[anchor].priority)
            anchor = index;
    }
    if (anchor >= 0 && tokens[anchor].priority >= 80)
        selected.add(anchor);
    ...
    if (compact.length > cap) {
        ...
        while (remaining > 0) {
            ...
            for (const position of growthOrder) {
                ...
                if (allocations[position] >= token.text.length)
                    continue;
 ...
 */
export function deriveEpisodeSummary(input, capChars = SUMMARY_CAP_CHARS) {
    const changelogs = (input.annotations ?? []).filter((a) => a.kind === 'changelog');
    if (changelogs.length > 0) {
        const head = changelogs[0].text.split('\n')[0].trim();
        if (head.length > 0)
            return truncateVerbatim(head, capChars);
    }
    ...
    const proseLines = (input.annotations ?? []).filter((a) => a.kind.startsWith('narration') || a.kind.startsWith('process:'));
    if (proseLines.length > 0) {
        ...
        const best = [...proseLines].sort((a, b) => ANNOTATION_PRIORITY[a.kind] - ANNOTATION_PRIORITY[b.kind])[0];
        const head = best.text.split('\n')[0].trim();
        if (head.length > 0)
            return truncateVerbatim(head, capChars);
    }
    const byPriority = [...input.members].sort((a, b) => TOUCH_KIND_RANK[b.touchKind] - TOUCH_KIND_RANK[a.touchKind]
        || b.touchCount - a.touchCount
        || comparePaths(a.path, b.path));
 ...
 */
export function selectVoiceInlays(annotations, max = 2) {
    const chosen = [...annotations]
        .sort((a, b) => ANNOTATION_PRIORITY[a.kind] - ANNOTATION_PRIORITY[b.kind]
 ...
 */
function voiceTimeSuffix(annotation) {
 ...
 */
export function renderEpisodeVoiceLines(episode, opts, maxLines = 2) {
    const voiceLabel = episodeAuthorLabel(episode, opts);
    const voiceLines = selectVoiceInlays(episode.annotations, maxLines).map((inlay) => renderVoiceLine(inlay, voiceLabel));
    const intent = episode.intent ?? null;
    return { voiceLines, intent };
}
function renderMembersLine(episode) {
    const ordered = [...episode.members].sort((a, b) => b.touchCount - a.touchCount
...
export function formatPointerLine(episode) {
    const range = episodeEventRange(episode);
    return `  ⌖ verbatim: ${episode.instanceId} events ${range.first}..${range.last}`;
}
function renderChapterBody(episode, sinceDeltas, maxVoiceInlays, voiceLabel) {
    const lines = [];
    ...
    if (episode.intent)
        lines.push(`  ↳ ask:"${truncateVerbatim(episode.intent, INTENT_TEXT_CAP_CHARS)}"`);
    lines.push(renderMembersLine(episode));
    if (episode.trace.length > 0)
        lines.push(`  trace: ${episode.trace}`);
...
function warmLine(episode, opts) {
...
function fullPreviousChapterLines(episode, maxVoiceInlays, opts) {
    const voiceLabel = episodeAuthorLabel(episode, opts);
    ...
    if (isForeignChapter(episode, opts))
        lines.push(`  ↞ from ${voiceLabel} (peer lineage)`);
    lines.push(...renderChapterBody(episode, [], maxVoiceInlays, voiceLabel));
    return lines;
}
function cardLinePriority(line) {
    if (line.startsWith('  trace: ') && line.includes(' ⇢ '))
        return 100;
    if (line.startsWith('  ⭐gotcha:') || line.startsWith('  ⚠'))
        return 95;
...
function truncateCardLine(line, capChars) {
    const cap = Math.max(0, Math.floor(capChars));
    if (cap === 0)
        return '';
    if (line.length <= cap)
 ...
 */
function fitCardToBudget(header, body, pointer, budgetChars) {
    const budget = Number.isFinite(budgetChars) ? Math.max(0, Math.floor(budgetChars)) : CHAIN_CARD_DEFAULT_BUDGET_CHARS;
    if (budget === 0)
        return '';
    if (pointer.length >= budget)
        return truncateCardLine(pointer, budget);
 ...
 */
export function formatChainCard(chapters, targetPath, sinceDeltas, opts = {}) {
    ...
    if (opts.bookends?.before)
        bookendLines.push(`  ↞ before: ${opts.bookends.before}`);
    if (opts.bookends?.after)
        bookendLines.push(`  after: ↠ ${opts.bookends.after}`);
    ...
    while (rendered.length > budget && state.body.some((line) => line.startsWith('  Δ '))) {
        for (let idx = state.body.length - 1; idx >= 0; idx--) {
            if (state.body[idx].startsWith('  Δ ')) {
 ...
 */
export function formatWalkPromotionCard(chapter, position, sinceDeltas, opts = {}) {
    if (opts.selfLineageOnly === true && isForeignChapter(chapter, opts))
        return '';
    const budget = opts.charBudget ?? CHAIN_CARD_DEFAULT_BUDGET_CHARS;
    const maxVoice = opts.maxVoiceInlays ?? 2;
 ...
 */
function formatBreadcrumb(current, crumb) {
...
function walkSpineRole(chapter) {
    if (chapter.annotations.some((a) => a.kind === 'star:gotcha' || a.kind === 'narration:hazard'))
        return 'gotcha';
    if (chapter.annotations.some((a) => a.kind === 'star:pivot'))
        return 'pivot';
    if (chapter.annotations.some((a) => a.kind === 'star:decision'))
        return 'decision';
    return 'context';
}
/**
 ...
 */
function nonDegenerateGist(chapter) {
    const summary = (chapter.summary ?? '').trim();
    if (summary && !isDegenerateGist(summary))
        return summary;
...
function isDegenerateGist(value) {
    ...
    if (tokens.length === 0)
        return true;
    const pathLike = tokens.filter((t) => t.includes('/') || t.includes('\\') || /\.[a-z0-9]{1,6}$/i.test(t)).length;
    return pathLike >= Math.ceil(tokens.length * 0.8);
}
function formatRelativeWalkDelta(currentIso, spineIso) {
    const currentMs = Date.parse(currentIso);
    const spineMs = Date.parse(spineIso);
    if (!Number.isFinite(currentMs) || !Number.isFinite(spineMs))
        return formatEpisodeDate(spineIso);
    const diffMs = spineMs - currentMs;
    const prefix = diffMs <= 0 ? 'T-' : 'T+';
    const absMs = Math.abs(diffMs);
    const dayMs = 24 * 3_600_000;
    if (absMs >= dayMs)
        return `${prefix}${Math.max(1, Math.round(absMs / dayMs))}d`;
    const hourMs = 3_600_000;
    if (absMs >= hourMs)
        return `${prefix}${Math.max(1, Math.round(absMs / hourMs))}h`;
...
export const DEFAULT_EPISODIC_VALUE_WEIGHTS = {
...
export const DEFAULT_EPISODIC_VALUE_LEDGER_CONFIG = {
    enabled: true,
...
export function anchorKindScore(kind) {
    switch (kind) {
        case 'chain':
        case 'rail':
            return 1;
...
export function strongerEpisodicKind(a, b) {
    if (a === undefined)
        return b;
 ...
 */
export function scoreEpisodicZoneValue(targetPath, zone, context = {}, boundarySeq = zone.lastEngagedBoundary ?? 0, weights = DEFAULT_EPISODIC_VALUE_WEIGHTS) {
...
export function episodicValueTtlMultiplier(value, config) {
...
export function effectiveEpisodicZoneTtl(baseTtlBoundaries, value, config) {
    if (!config.enabled)
        return baseTtlBoundaries;
    return Math.max(1, Math.round(baseTtlBoundaries * episodicValueTtlMultiplier(value, config)));
}
export function createEpisodicInjectionState() {
    return {
        boundarySeq: 0,
        stash: null,
        zones: new Map(),
 ...
 */
export function expireEpisodicZones(state) {
 ...
 */
export function episodicServedTermKeys(state) {
    const keys = [];
    for (const targetPath of state.zones.keys()) {
        if (targetPath.startsWith('term:'))
            keys.push(targetPath);
    }
    return keys.sort();
}
function cloneEpisodicCard(card) {
    return {
...
function isActivePathAnchorCard(card) {
    return card.kind === 'chain' || card.kind === 'mention';
}
function zoneMatchesTouchedPath(targetPath, touched) {
 ...
 */
export function consumeEpisodicStash(state, maxAgeBoundaries = EPISODIC_STASH_MAX_AGE_BOUNDARIES) {
 ...
 */
const EPISODIC_EDIT_TOOL_HINTS = ['edit', 'write', 'apply_patch', 'notebookedit', 'str_replace', 'create_file'];
/**
 ...
 */
export function classifyEpisodicBoundaryTool(toolName) {
    if (typeof toolName !== 'string' || toolName.length === 0)
        return 'gated';
    const lower = toolName.toLowerCase();
    if (lower === 'task_rail' || lower.endsWith('__task_rail'))
        return 'rail_anchor';
    if (!lower.includes('todowrite') && EPISODIC_EDIT_TOOL_HINTS.some((hint) => lower.includes(hint)))
        return 'edit';
    return 'gated';
}
/**
 ...
 */
export function resolveEpisodicReadRateBoundaries(raw) {
    if (raw === undefined || raw.trim() === '')
        return EPISODIC_READ_RATE_DEFAULT_BOUNDARIES;
 ...
 */
export function consumeEpisodicStashRateGated(state, options) {
    ...
    if (state.episodicGatedBoundariesSinceServe < rate) {
        if (state.stash)
            state.episodicRateHeld++;
        return null;
    }
    const cards = suppressEpisodicCardsAlreadyInPov(state, consumeEpisodicStash(state, options.maxAgeBoundaries), options.providerPovText ?? '');
...
function normalizeEpisodicPovText(text) {
...
function cleanEpisodicPovProbe(text) {
    return text.trim().replace(/(?:\.{3}|…)+$/, '').trim();
}
function episodicCardPovProbes(card) {
    const probes = new Set();
    const add = (raw) => {
        if (!raw)
            return;
        const cleaned = cleanEpisodicPovProbe(raw);
        if (cleaned.length < EPISODIC_POV_PROBE_MIN_CHARS || isDegenerateGist(cleaned))
            return;
...
export function episodicCardAlreadyInPov(card, providerPovText) {
    if (!providerPovText)
        return false;
 ...
 */
export function suppressEpisodicCardsAlreadyInPov(state, cards, providerPovText) {
    if (!cards || cards.length === 0 || !providerPovText)
 ...
 */
export function noteEpisodicInjection(state, cards, ttlBoundaries = EPISODIC_ZONE_TTL_BOUNDARIES, valueOptions) {
    ...
    for (const card of cards) {
        ...
        for (const id of card.chapterIds)
            merged.add(id);
        ...
        if (config?.enabled) {
            const value = scoreEpisodicZoneValue(card.targetPath, zone, valueOptions?.context, state.boundarySeq, valueOptions?.weights);
        ...
        if (card.kind === 'pointer')
            state.completedChainTargetPaths.add(card.targetPath);
        else
            state.completedChainTargetPaths.delete(card.targetPath);
        if (card.kind === 'mention')
            state.episodeCardsInjected++;
        else
            state.chainCardsInjected++;
 ...
 */
export function refreshEpisodicZones(state, touchPaths, ttlBoundaries = EPISODIC_ZONE_TTL_BOUNDARIES, valueOptions) {
    ...
    for (const [targetPath, zone] of state.zones) {
        if (zoneMatchesTouchedPath(targetPath, touched)) {
            ...
            const ttl = config?.enabled
                ? effectiveEpisodicZoneTtl(ttlBoundaries, scoreEpisodicZoneValue(targetPath, zone, valueOptions?.context, state.boundarySeq, valueOptions?.weights), config)
                : ttlBoundaries;
 ...
 */
export function episodicCardHeaderLine(card) {
 ...
 */
export function collectResidentEpisodicHeaders(viewText) {
    const out = new Set();
    if (!viewText || viewText.indexOf('[Episode ') === -1)
        return out;
    for (const line of viewText.split('\n')) {
        if (EPISODIC_CARD_HEADER_PREFIX_RE.test(line))
            out.add(line);
    }
    return out;
}
 ...
 */
export function reconcileVisibleEpisodicHeaders(state, residentHeaders) {
...
export function reconcileVisibleEpisodicView(state, view) {
    const headers = new Set();
    for (const message of view) {
        if (typeof message.content === 'string') {
            for (const header of collectResidentEpisodicHeaders(message.content))
                headers.add(header);
            continue;
        }
        ...
        for (const block of message.content) {
            if (block?.type !== 'text' || typeof block.text !== 'string')
                continue;
            for (const header of collectResidentEpisodicHeaders(block.text))
                headers.add(header);
        }
    }
 ...
 */
export const EPISODIC_BOOKKEEPING_TOOLS = new Set([
    'task_rail',
    'partner_claim_file',
...
export function isEpisodicBookkeepingTool(toolName) {
 ...
 */
export function activeEpisodicPathCards(state, touchPaths, options = {}) {
    return selectActiveEpisodicPathCards(state, touchPaths, options).cards;
}
export function selectActiveEpisodicPathCards(state, touchPaths, options = {}) {
    ...
    // can pass below.
    if (touchPaths.length === 0 && !repinInactive)
        return empty();
    const touched = new Set(touchPaths);
    ...
    for (const [targetPath, zone] of state.zones) {
        ...
        const value = enabled
            ...
            : 0;
        const reason = isTouched ? 'touched' : 'inactive';
...
function compactActivePathCard(card, budget) {
    if (budget <= 0)
        return null;
    if (card.renderedCard.length <= budget)
        return card.renderedCard;
    const lines = card.renderedCard.split('\n');
    const header = lines[0] ?? '';
    const members = lines.find((line) => line.startsWith('  members: '));
    const pointer = lines.find((line) => line.startsWith('  ⌖ verbatim: '));
    const compact = [header, members, pointer].filter((line) => typeof line === 'string' && line.length > 0).join('\n');
    if (compact.length === 0)
        return null;
 ...
 */
export function renderActiveEpisodicPathBlock(cards, syntheticPrefix, options = {}) {
    if (cards.length === 0)
        return null;
 ...
 */
export const EPISODIC_NARRATION_REMINDER = '[🗣 above is recovered agent voice — it survived into memory because an agent tagged its messages by register. Open yours with one of 🔍 working · ▶ executing · 🏁 verdict · ⚠️ hazard · ❓ blocked and future agents inherit your conclusions the same way.]';
/**
 ...
 */
export function formatEpisodicCardProvenance(card) {
 ...
 */
export function renderEpisodicBoundaryBlock(cards, syntheticPrefix, counterFooter, narrationReminder) {
    if (cards.length === 0)
        return null;
 ...
 */
export function extractEpisodeMentionPaths(texts, isSyntheticLine, cap = 8) {
    const tokenPattern = /(?:\/|~\/)?(?:[\w.()[\]-]+\/)*[\w()[\]-]+(?:\.[\w-]+)+/g;
    const found = new Set();
    for (const text of texts) {
        for (const line of text.split('\n')) {
            if (isSyntheticLine(line))
                continue;
            for (const match of line.matchAll(tokenPattern)) {
                let token = match[0];
                if (token.startsWith('./'))
                    token = token.slice(2);
                if (isEpisodeMemberPath(token))
                    found.add(token);
            }
            if (found.size >= cap * 4)
                break;
...
//# sourceMappingURL=foldEpisodes.js.map